define( [ 'ifuture', 'three' ], function( ifuture, THREE ) {
    ...
    var HOUSE_LIGHTS = {
        ...
        // Copy from SweetHomeJs
        solids: [
            // [ [ 0.9, 0.9, 0.9 ], [ 1.732, -0.8, -1 ], 0.618 ],
            [ [ 0.6, 0.6, 0.6 ], [ 1.732, -0.8, -1 ], 0.162 ],
            [ [ 0.9, 0.9, 0.9 ], [ - 1.732, -0.8, -1 ], 0.618 ],
            [ [ 0.98, 0.98, 0.98 ], [ 0, -0.8, 1 ], 0.618 ],
            [ [ 0.99, 0.99, 0.99 ], [ 0, 1, 0 ], 0.618 ]
        ]
    };

    var fishSpotLightOptions = {
        color: 0x2f1463,
        intensity: 6.0,
        angle: 45,
        penumbra: 0,
    ...
    function createSolidVisualization ( angle, distance ) {
        ...
        for ( var i = 0; i < 10; i ++ ) {
            var x = 0.1 * i;
            points.push( new THREE.Vector2( x, x * x ) );
        }
    ...
    var Solid = function ( app, opt_options ) {
        ...
        directionLight.position.fromArray( HOUSE_LIGHTS.directions[ 0 ].position );

        this._lightGroup = new THREE.Group();
        this._lightGroup.add( ambientLight, directionLight );

        for ( var i = 0; i < HOUSE_LIGHTS.solids.length ; i ++ ) {
            var data = HOUSE_LIGHTS.solids[ i ];
            var light = new THREE.DirectionalLight( new THREE.Color().fromArray( data[ 0 ] ), data[ 2 ] );
            light.position.fromArray( data[ 1 ] );
            this._lightGroup.add( light );
            // if ( debugMode ) {
            //     var helper = new THREE.DirectionalLightHelper( light, 100 * 3 );
            //     scene.add( helper );
            // }
        }

        this._visionHelper = createSolidVisualization( fishSpotLightOptions.angle / 180 * Math.PI, fishSpotLightOptions.distance );
        this._visionHelper.visible = false;
        this._visionHelper.renderOrder = 1;

        var targetElement = options.target;
        var canvas = document.createElement( 'CANVAS' );
        canvas.className = 'dx-three dx-page';
	...
	this._renderer.setSize( window.innerWidth, window.innerHeight );
        ...
        this._mtlLoader = new THREE.MTLLoader( manager );
        this._objLoader = new THREE.OBJLoader( manager );

        window.addEventListener('resize', Solid.prototype.onDocumentResize_.bind( this ), false);
        // window.addEventListener('orientationchange', Solid.prototype.onDocumentResize_.bind( this ), false);
    ...
    Solid.prototype.onDocumentResize_ = function ( e ) {
        ...
        var viewAspect = w / h;
        this._camera.aspect =  viewAspect;
    ...
    Solid.prototype.setVisible = function ( value ) {
        this._visible = value;
        this._control.enabled = value;
        this._renderer.domElement.style.display = value ? '' : 'none';
    };

    Solid.prototype.isVisible = function () {
        return !! this._visible;
    };

    Solid.prototype.render = function () {

        if ( this._renderer && this._visible && this._scene ) {
    ...
    Solid.prototype.loadHouse = function ( url, origin, extent ) {
        ...
        this.loadHouseObj_( url, callback );
    };
    ...
    Solid.prototype.getExtentFromCamera_ = function ( origin ) {
        ...
        var dy = d * Math.tan( camera.fov / 2 * Math.PI / 180 );
    ...
    Solid.prototype.resetHouseCamera_ = function () {
        ...
        var x0 = ( min.x + max.x ) / 2;
        var y0 = ( min.y + max.y ) / 2;
        var z0 = min.z;
        var z1 = max.z;
    ...
    /**
     * Load house blender object.
     * @private
     * @param {string} path - mtl path.
     * @param {string} name - mtl name.
     */
    Solid.prototype.loadHouseObj_ = function ( url, callback ) {

        var scope = this;
        var i = url.lastIndexOf( '/' ) + 1;
        var name = url.slice( i ).split( '.' )[ 0 ]
        var path = url.slice( 0, i );

        var onProgress = function ( xhr ) {
            if ( xhr.lengthComputable ) {
                var percentComplete = xhr.loaded / xhr.total * 100;
                // fivehouse.setHouseProgress( Math.round( percentComplete, 2 ) );
        ...
        var onLoad = function ( object ) {
            ...
            //     console.log( 'Solid house center at ' + b.center.x + ', ' + b.center.y + ', ' + b.center.z );
            //     console.log( 'Solid house bounding radius is ' + b.radius );
            //     b = g.boundingBox.max;
            //     console.log( 'Solid house bounding max:  ' + b.x + ', ' + b.y + ', ' + b.z );
            //     b = g.boundingBox.min;
            //     console.log( 'Solid house bounding minx:  ' + b.x + ', ' + b.y + ', ' + b.z );
            // }

        };

        var mtlLoader = this._mtlLoader;
        ...
        mtlLoader.load( name + '.mtl', function( materials ) {
            ...
            objLoader.load( name + '.obj', onLoad, onProgress, onError );
